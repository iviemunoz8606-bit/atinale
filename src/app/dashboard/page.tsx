...
export default function Dashboard() {
  ...
  async function loadDashboard() {
    try {
      ...
      // 4. Mis quinielas (donde soy miembro)
      const { data: memberData } = await supabase
        .from('pool_members')
        .select(`
          id, pool_id, points, rank, payment_status,
          pool:pools(*)
        `)
      ...
      const { data: predsData } = await supabase
        .from('predictions')
        .select(`
          id, predicted_home, predicted_away, points_earned,
          match:matches(home_team, away_team, home_flag, away_flag, scheduled_at, status, home_score, away_score)
        `)
        .eq('user_id', session.user.id)
      ...
      // 6. Leaderboard de la primera quiniela activa
      if (poolsData && poolsData.length > 0) {
        const firstPool = poolsData[0]
        setActivePoolId(firstPool.id)
        loadLeaderboard(firstPool.id)
      }
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <p style={{ color: '#6B7280', fontSize: 14, fontFamily: 'sans-serif' }}>Cargando tu dashboard...</p>
  ...
  const myRank = myPools.length > 0
    ? Math.min(...myPools.map(m => m.rank || 999))
    : null
    ...
    <div style={{ background: '#0A0D12', minHeight: '100vh', fontFamily: "'Outfit', 'Helvetica Neue', sans-serif", color: '#F0F2F8' }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-item:hover .nav-label { color: #F5B731 !important; }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {user.avatar_url ? (
            <img src={user.avatar_url} alt={user.name}
              style={{ width: 38, height: 38, borderRadius: '50%', border: '2px solid rgba(245,183,49,0.3)', objectFit: 'cover' }} />
          ) : (
            <div style={{
              width: 38, height: 38, borderRadius: '50%',
              background: 'linear-gradient(135deg, #F5B731, #00C46A)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontWeight: 700, fontSize: 16, color: '#0A0D12',
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px 100px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 24 }}>
          {[
            ...
            { label: 'Quinielas', value: user.total_quinelas || myPools.length, color: '#4FADFF', sub: 'activas' },
          ].map((stat, i) => (
            <div key={i} style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          {(['disponibles', 'mis-quinielas'] as const).map(tab => (
            <button key={tab} className="tab-btn" onClick={() => setActiveTab(tab)} style={{
              ...
              color: activeTab === tab ? '#0A0D12' : '#6B7280',
              transition: 'all 0.2s'
            }}>
        ...
        {activeTab === 'disponibles' && (
          <div style={{ animation: 'fadeUp 0.3s ease both' }}>
            {pools.length === 0 ? (
              <div style={{
                background: '#111520', border: '1px solid rgba(255,255,255,0.07)',
                borderRadius: 18, padding: 32, textAlign: 'center', color: '#6B7280'
              }}>
              ...
              pools.map((pool, i) => {
                ...
                const netPrize = pool.total_pot - Math.round(pool.total_pot * 0.1)
                const commission = Math.round(pool.total_pot * 0.1)
                const fillPct = Math.min(100, (pool.current_participants / pool.max_participants) * 100)
                const isMember = myPools.some(m => m.pool_id === pool.id)
                ...
                return (
                  <div key={pool.id} className="pool-card" style={{
                    ...
                    <div style={{
                      ...
                      background: `linear-gradient(135deg, ${comp.bg}, transparent)`,
                      borderBottom: '1px solid rgba(255,255,255,0.07)'
                    }}>
                      <div style={{
                        width: 44, height: 44, borderRadius: 12, flexShrink: 0,
                        background: `linear-gradient(135deg, ${comp.color}, ${comp.color}99)`,
                      ...
                      <div style={{
                        ...
                        background: pool.status === 'open' ? 'rgba(0,196,106,0.15)' : 'rgba(245,183,49,0.15)',
                        color: pool.status === 'open' ? '#00C46A' : '#F5B731',
                        border: `1px solid ${pool.status === 'open' ? 'rgba(0,196,106,0.3)' : 'rgba(245,183,49,0.3)'}`
                      }}>
                    ...
                    <div style={{ padding: '14px 16px' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 24, color: '#F5B731', lineHeight: 1 }}>
                            {formatMXN(pool.total_pot || pool.current_participants * pool.entry_fee)}
                          </div>
                      ...
                      <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 4, height: 4, marginBottom: 6, overflow: 'hidden' }}>
                        <div style={{
                          ...
                          background: 'linear-gradient(90deg, #00C46A, #F5B731)',
                          position: 'relative', overflow: 'hidden', transition: 'width 1s ease'
                        }}>
                          <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{ fontSize: 12, color: '#6B7280' }}>
                        {isMember ? '✅ Ya participas' : `${pool.max_participants - pool.current_participants} lugares disponibles`}
                      </div>
                      <Link href={`/quiniela/${pool.id}`} style={{ textDecoration: 'none' }}>
                        <button className="btn-predict" style={{
                          padding: '9px 20px', borderRadius: 20,
                          background: isMember ? 'linear-gradient(135deg, #00C46A, #00864A)' : 'linear-gradient(135deg, #F5B731, #C9930A)',
                          color: '#0A0D12', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer',
                          boxShadow: '0 4px 15px rgba(245,183,49,0.3)', transition: 'all 0.15s',
                          fontFamily: "'Outfit', sans-serif"
                        }}>
                          {isMember ? 'VER PREDICCIONES →' : 'PREDECIR →'}
                        </button>
                      </Link>
                    </div>
                  </div>
            ...
            {myPools.length === 0 ? (
              <div style={{
                ...
                <button onClick={() => setActiveTab('disponibles')} style={{
                  ...
                  color: '#0A0D12', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer',
                  fontFamily: "'Outfit', sans-serif"
                }}>Ver quinielas disponibles</button>
              </div>
            ) : (
                ...
                <div key={member.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <div style={{
                      ...
                      background: member.payment_status === 'approved' ? 'rgba(0,196,106,0.15)' : 'rgba(245,183,49,0.15)',
                      color: member.payment_status === 'approved' ? '#00C46A' : '#F5B731'
                    }}>
                  ...
                  <div style={{ display: 'flex', gap: 16 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 28, color: '#00C46A' }}>
        ...
        {recentPredictions.length > 0 && (
          <div style={{ marginTop: 8 }}>
            <div style={{
              fontSize: 11, color: '#6B7280', textTransform: 'uppercase', letterSpacing: 2,
            ...
            {recentPredictions.map((pred, i) => {
              ...
              return (
                <div key={pred.id} className="pred-card" style={{
                  background: '#111520', border: '1px solid rgba(255,255,255,0.07)',
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{
                      ...
                      background: isExact ? 'rgba(245,183,49,0.15)' : isCorrect ? 'rgba(0,196,106,0.15)' : 'rgba(107,114,128,0.12)',
                      color: isExact ? '#F5B731' : isCorrect ? '#00C46A' : '#6B7280'
                    }}>
        ...
        {pools.length > 0 && (
          <div style={{ marginTop: 8 }}>
            ...
            {/* Selector de quiniela si hay varias */}
            {pools.length > 1 && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 12, overflowX: 'auto', paddingBottom: 4 }}>
                {pools.map(pool => (
                  <button key={pool.id} onClick={() => loadLeaderboard(pool.id)} style={{
                    padding: '6px 14px', borderRadius: 20, border: 'none', cursor: 'pointer', whiteSpace: 'nowrap',
                    background: activePoolId === pool.id ? 'rgba(245,183,49,0.15)' : 'rgba(255,255,255,0.04)',
                    color: activePoolId === pool.id ? '#F5B731' : '#6B7280',
            ...
            <div style={{
              background: '#111520', border: '1px solid rgba(255,255,255,0.07)',
              borderRadius: 18, overflow: 'hidden', animation: 'fadeUp 0.5s ease both'
            }}>
              {/* Header leaderboard */}
              <div style={{
                padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                borderBottom: '1px solid rgba(255,255,255,0.07)',
                background: 'linear-gradient(135deg, rgba(245,183,49,0.06), transparent)'
              }}>
                <div style={{ fontWeight: 700, fontSize: 15 }}>
                  {pools.find(p => p.id === activePoolId)?.name || 'Ranking'}
                </div>
                ...
                leaderboard.map((entry, i) => {
                  ...
                  return (
                    <div key={entry.user_id} style={{
                      ...
                      background: isMe ? 'rgba(245,183,49,0.05)' : 'transparent',
                      borderLeft: isMe ? '2px solid #F5B731' : '2px solid transparent'
                    }}>
                      <div style={{
                        fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, width: 28, textAlign: 'center',
                        color: i < 3 ? '#F5B731' : '#6B7280'
                      }}>{i + 1}</div>
                      <div style={{
                        width: 36, height: 36, borderRadius: '50%', flexShrink: 0,
                        background: `linear-gradient(135deg, ${avatarColors[i % avatarColors.length]})`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontWeight: 700, fontSize: 14,
                        color: isMe ? '#0A0D12' : 'white'
                      }}>
                      ...
                      <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, color: '#F5B731' }}>
                        {entry.points}
                      </div>
                    </div>
                  )
          ...
          <Link key={item.label} href={item.href} style={{ flex: 1, textDecoration: 'none' }}>
            <div className="nav-item" style={{
              ...
              <div className="nav-label" style={{
                fontSize: 10, color: item.active ? '#F5B731' : '#6B7280',
